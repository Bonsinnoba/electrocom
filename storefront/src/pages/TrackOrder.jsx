...
export default function TrackOrder() {
  ...
  const getStatusIcon = (status) => {
      switch(status) {
          ...
          case 'cancelled': return <CheckCircle size={20} />; // Using check but coloring it red below
    ...
    <div style={{ padding: '60px 20px', minHeight: '80vh', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ maxWidth: '800px', width: '100%' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            <h1 style={{ fontSize: '36px', fontWeight: 800, marginBottom: '12px', background: 'linear-gradient(135deg, var(--primary-blue), var(--accent-blue))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Track Your Order</h1>
          ...
          <div className="card glass animate-fade-in" style={{ padding: '32px', borderRadius: '24px', marginBottom: '40px' }}>
            <form onSubmit={handleTrack} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '16px', alignItems: 'flex-end' }}>
              ...
              <button 
                ...
                disabled={loading}
                style={{ padding: '14px 32px', borderRadius: '12px', height: '50px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 700 }}
              >
            ...
            <div className="card glass animate-fade-up" style={{ padding: '32px', borderRadius: '24px', animationDelay: '0.1s' }}>
              ...
              <div style={{ padding: '24px 0 40px 0' }}>
                 <div style={{ display: 'flex', justifyContent: 'space-between', position: 'relative', maxWidth: '600px', margin: '0 auto' }}>
                    ...
                    {/* Background Line */}
                    <div style={{ position: 'absolute', top: '24px', left: '10%', right: '10%', height: '4px', background: 'var(--border-light)', zIndex: 1, borderRadius: '4px' }}>
                        {/* Progress Fill */}
                        <div style={{ 
                            height: '100%', 
                    ...
                    {orderData.timeline && orderData.timeline.map((step, idx) => (
                        <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2, flex: 1 }}>
                            <div style={{ 
                                ...
                                border: `4px solid ${step.completed ? 'var(--bg-surface)' : 'var(--border-light)'}`,
                                display: 'flex', 
                                alignItems: 'center', 
                                ...
                                {idx === 0 ? <CheckCircle size={20}/> : 
                                 idx === 1 ? <Package size={20}/> : 
                                 idx === 2 ? <TrendingUp size={20}/> : 
                                 <CheckCircle size={20}/>}
                            </div>
                            <span style={{ fontSize: '13px', fontWeight: step.completed ? 700 : 500, color: step.completed ? 'var(--text-main)' : 'var(--text-muted)', marginBottom: '4px', whiteSpace: 'nowrap' }}>{step.label}</span>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {orderData.items && orderData.items.map((item, idx) => (
                      <div key={idx} style={{ display: 'flex', gap: '16px', alignItems: 'center', padding: '12px', background: 'var(--bg-surface-secondary)', borderRadius: '12px' }}>
                         <div style={{ width: '60px', height: '60px', borderRadius: '8px', background: 'white', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            {item.image_url ? (
